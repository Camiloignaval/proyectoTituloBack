...
const obtenerAforoPorDia = async (req, res) => {
  ...
  try {
    ...
    const arrayDevolver = horasReservadas.map((a) => ({
      hora: a.hora.slice(0, 5),
      aforo: Number(aforoDia?.aforo) - Number(a.count),
    }));
    res.status(200).json({
      dataDia: aforoDia,
...
const selectRoutine = async (req, res) => {
  try {
    await selectRoutineActive(Object.values(req.body));
    res.status(200).json({
      ok: true,
      msg: "Rutina activada en 'Mi rutina'",
    });
  } catch (error) {
    console.log(error);
    res.status(400).json({
